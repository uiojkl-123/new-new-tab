...
*/
const openDatabase = (name = 'default', storeNames) =>
...
const db64 = {
  ...
  use: (name, storeName) => {
    ...
    return {
      set: async (key, value) =>
        openDatabase(name, [storeName]).then(database => setData(database, storeName, key, value)),
      setEntries: async value =>
        openDatabase(name, storeName).then(database => setData(database, storeName, null, value, 'entries')),
